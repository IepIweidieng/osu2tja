...
- TJA 音符定义
  ...
  - 音符符号
    ...
    - [x] 重叠同轨音符 → 填 `0` 放被重叠音符终点（若需要），再用负 `#DELAY` 回到重叠音符起点来放置起点。跨时间点时再用另外的 `#DELAY` 修正计时 。（新功能）
    - [x] 同时不同轨音符 → 单个组合音符（新功能）
      - 两咚 `1`/`3` + `1`/`3` → `3`（大咚）
      - 两咔 `2`/`4` + `2`/`4` → `4`（大咔）
...
- TJA 命令
  ...
  - [x] 负的 (小节长 ÷ BPM) 或大的负 `#DELAY` → 非完全递增时间序的谱面事件，依时间重新排序（新功能）
    - 被覆盖的小节的小节线会转换成最上层小节上的可见小节线。（改进）
  - [x] `#DELAY` → 移动谱面定义游标的时间
    - 也会移动 `#DELAY` 之后的可见小节线的时间。（改进）
  ...
  - [x] 可见小节线（含 `#BARLINE`）→ 非继承时间点：不隐藏首个小节线 + 不完整小节，或小节完整不需转换（新功能）
  - [x] 不可见小节线（隐藏的、或由于 `#DELAY` 或 osu! 计时舍入造成的不预期的）→ 非继承时间点：隐藏首个小节线 + （可能不完整的）小节（新功能）
